...
def sum_of_minmax_differences(input_list_of_lists):
    result = 0
    for list in input_list_of_lists:
        result += max(list) - min(list)
    return result

def sum_evenly_divisible(input_list_of_lists):
    result = 0
    for sublist in input_list_of_lists:
        for itr in range(0,len(sublist)):
            tmp_list = list(set(sublist) - set([sublist[itr]]))
            for elem in tmp_list:
                if 0 == sublist[itr] % elem :
                    result = result + sublist[itr] / elem
                    break
    return result

#TODO: work on a method to fetch the input direclty from the webpage
if __name__ == '__main__':
    ...
    print("Sum of max min differences is : " + str(sum_of_minmax_differences(input)))
    print("Sum of evenly divisible is : " + str(sum_evenly_divisible(input)))
